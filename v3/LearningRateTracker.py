...
class LearningMetricsTracker:
    ...
    """
    def log_round_metrics(self, round_num: int, results: Dict, learning_rate: float, exploration_rate: float = 0.0, threshold: float = 0.0):
        ...
        self.metrics['r2_thresholds'].append(threshold)
    ...
    def plot_learning_curves(self, save_path: Optional[str] = None, experiment_config: Optional[Dict] = None):
        ...
        # 6. Model R2 scores with stored thresholds
        ax = axes[1, 2]
        ...
        for round_idx, r2_scores in enumerate(self.metrics['model_r2_scores']):
            if r2_scores:
                ...
                threshold = self.metrics['r2_thresholds'][round_idx] if round_idx < len(self.metrics['r2_thresholds']) else 0
                
                # Filter and plot scores with color coding
        ...
        # Plot threshold line from stored values
        if self.metrics['r2_thresholds'] and self.metrics['round']:
            ax.step(self.metrics['round'], self.metrics['r2_thresholds'], 
                    where='post', color='blue', linestyle='-', linewidth=2, 
                    alpha=0.7, label='R2 Threshold')
