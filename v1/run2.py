...
def create_warmup_dataset(oracle_fn, vocab_size: int, max_seq_len: int, n_samples: int = 50):
    ...
    for _ in range(n_samples):
        # Generate with varying patterns
        ...
        if seq_type < 0.25:
            ...
            # Alternating patterns with noise
...
def run_dyna_ppo_algorithm(oracle_fn, vocab_size: int, max_seq_len: int, 
    ...
    if use_warmup:
        ...
        if hasattr(dyna_ppo, 'fit_surrogate_models_improved'):
            print("\nPre-training surrogate models on warm-up data...")
            initial_models, initial_scores = dyna_ppo.fit_surrogate_models_improved(
                warmup_sequences, warmup_rewards, round_num=0
            )
            print(f"Initial models trained: {len(initial_models)}")
            if initial_scores:
                print(f"Initial R² scores - Mean: {np.mean(initial_scores):.3f}, "
    ...
    for n in range(1, N + 1):
        ...
        # Display model R² scores if available
        if results.get('model_r2_scores'):
            r2_scores = results['model_r2_scores']
            if r2_scores:
                print(f"  Model R² - Mean: {np.mean(r2_scores):.3f}, "
...
if __name__ == "__main__":
    ...
    trained_dyna_ppo = run_dyna_ppo_algorithm(
        ...
        max_seq_len=8,    # 12-base sequences
        N=15,              # More rounds for better learning
